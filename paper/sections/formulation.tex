...
tests, execution-match of SQL, or exact/label match for QA and diagnosis. Gold
...
\paragraph{Why this setting is hard.} The agent gets one attempt per problem, a single bit per attempt, no gold
text, and no second pass over the stream.
Improvements must be bootstrapped from the agent's own verified outputs, and
any weight update risks destabilizing the remainder of the stream. The RL
baseline in \S\ref{sec:exp-fused} realizes this risk.
