...
\item \textbf{Online vs.\ batch} (\S\ref{sec:exp-transfer}): with loss, data,
  model, and oracle budget held fixed, a single streaming pass matches
  multi-epoch batch self-distillation on both source mastery and transfer to
  an unseen domain; without matched exploration, batch trails by $5.5$
  points. Coverage, not the schedule itself, is the operative mechanism---and
  the stream achieves it in one pass, without the corpus upfront.
